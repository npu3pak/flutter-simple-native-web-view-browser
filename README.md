...
```yaml
dependencies:
  simple_native_web_view_browser: ^0.5.0
```
...
Обработчик `onClosed` гарантированно вызывается после закрытия браузера любым способом. Если нативное событие о закрытии потеряно (например, при детаче Flutter-движка с открытым браузером), сессия закрывается принудительно через несколько секунд после `close()`, и `onClosed` всё равно срабатывает.
...
- **`usePersistentCookieStore`** — нужно ли сохранять куки между запусками приложения. Тип — `bool`. Значение по умолчанию — `true`. При `true` куки остаются после закрытия браузера и перезапуска приложения; при `false` используется эфемерный режим, поведение которого зависит от платформы (см. «Постоянное и эфемерное хранилище»).
...
- **`enableCookiesAndroid`** — разрешить установку и передачу кук. Тип — `bool`. Значение по умолчанию — `true`. Действует только на Android (`CookieManager.setAcceptCookie`); на iOS куки WebView не отключаются. Отключение применяется к процессу Android целиком, поэтому может повлиять на другие WebView приложения; после закрытия браузера приём кук возвращается в исходное состояние. При `false` отключаются только куки: JavaScript и хранилища страниц (localStorage и др.) продолжают работать.
...
- **`isSharingAvailable`** — показывать стандартную кнопку «Поделиться» на верхней панели справа от заголовка/адресной строки. Тип — `bool`. Значение по умолчанию — `false`. Кнопка открывает системный шеринг и передаёт ему адрес текущей страницы браузера.
...
- **`onDownloadStart`** — обработчик, который вызывается, когда пользователь пытается скачать файл (например, переходит по ссылке с `Content-Disposition: attachment` или на бинарный файл). Тип — `void Function(Uri url)`. По умолчанию не задан. Браузер не выполняет загрузку: он передаёт приложению адрес файла и отменяет переход на него — скачивание выполняет само приложение (например, через `package:http` или нативный загрузчик).
...
| `name` | Имя куки. |
| `value` | Значение куки. |
| `domain` | Домен, для которого действует кука. Если не указан, берётся домен страницы: стартовой — для `initialCookies`, текущей — для `reloadWithCookies`. |
...
Если в куке не задан `domain`, он берётся из текущего адреса страницы. Учитывайте это после редиректа на другой домен: если страница перенаправлена, а кука должна применяться к новому домену, укажите его в поле `domain` явно.

### Постоянное и эфемерное хранилище

Параметр `usePersistentCookieStore` определяет, что происходит с куками:
...
- `false` — эфемерный режим. Поведение зависит от платформы:
  - **iOS** — используется изолированное (non-persistent) хранилище: куки сеанса живут только в памяти и исчезают после закрытия браузера;
  - **Android** — хранилище кук общее для всего процесса, эфемерный режим не создаёт изолированного хранилища. При открытии браузера общие куки очищаются, затем куки сеанса пишутся в общее хранилище и остаются в нём после закрытия браузера — до следующего открытия в эфемерном режиме или ручной очистки приложением.

Особенность Android: хранилище кук общее для всех браузеров приложения. Эфемерный режим очищает куки всех браузеров при открытии, поэтому при его использовании могут завершиться сеансы других экранов приложения с браузером.
...
### Попытка скачать файл — `onDownloadStart`

Обработчик вызывается, когда пользователь пытается скачать файл. Загрузка — не функция браузера: переход на файл отменяется, а приложению передаётся его адрес.

```dart
onDownloadStart: (url) {
  print('Пользователь хочет скачать файл: $url');
  // Загрузку выполняет приложение.
},
```
...
Демонстрационное приложение (папка `example`) содержит две страницы:

- **Минимальный пример** (`minimal_example_page.dart`) — только адрес, кнопки
  «Открыть»/«Закрыть» и строка состояния с последним событием. Все параметры
  запроса — значения по умолчанию. Подходит для быстрого знакомства с плагином.
- **Демо-стенд** (`browser_demo_screen.dart`) — отладочная страница с полным
  набором параметров: пользовательский агент, куки, режимы адресной строки,
  редиректы на кастомные схемы, журнал событий.

Переход между страницами — по кнопке «Минимальный пример» в верхней панели
демо-стенда.
...
- `SimpleBrowserReopenPolicy.replaceCallbacksAndSettings` — сессия заменяется: привязываются обработчики нового запроса и применяются его настройки — пользовательский агент, режим адресной строки, режим отладки, приём кук (`enableCookiesAndroid`), доступ к файлам (`allowFileAccess`), кнопка «Поделиться» (`isSharingAvailable`); начальные куки устанавливаются в хранилище и вступят в силу при следующей загрузке страницы. Страница не перезагружается.
...
Адрес с кастомной схемой доставляется приложению **один раз за сеанс браузера**: повторная попытка открыть кастомную схему в том же сеансе молча игнорируется (одна и та же навигация может перехватываться несколькими путями — кликом, серверным редиректом, ошибкой). Ограничение сбрасывается при повторном открытии браузера (reopen). Если приложению нужны несколько редиректов подряд, закрывайте браузер после обработки первого.
...
### Загружаемые схемы `data:`, `about:`, `blob:`

Плагин разрешает загрузку страниц со схемами `data:`, `about:` и `blob:` помимо `http:`/`https:` — они используются для встроенного HTML-контента и демонстрационных сценариев. Такие адреса не считаются кастомными схемами и не передаются через `onSchemeRedirect`; вводить их в адресной строке можно (например, `data:text/html,...`). Используйте эту возможность только для доверенного контента.
