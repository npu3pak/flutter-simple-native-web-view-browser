...
- **`onSchemeRedirect`** — обработчик, который вызывается при попытке браузера открыть адрес с кастомной схемой (например, `myapp://...`). Тип — `void Function(Uri url)`. По умолчанию не задан. Обработчик получает адрес с кастомной схемой. Подробнее — в разделе «События».
...
Частая задача — узнать, что браузер перешёл на определённый адрес (например, когда сайт возвращает приложение после входа). Для этого проверяйте адрес в обработчиках `onLoadStop`, `onLoadError` и `onSchemeRedirect` на совпадение с нужным началом адреса.
...
- Кастомные схемы адресов (например, `myapp://...`) не загружаются в браузере. Переход на такой адрес передаётся приложению через обработчик `onSchemeRedirect` — смотрите раздел «События».
