...
- Без изменений в Android-реализации (версия синхронизирована с ядром).

## 0.3.0

- Обработчик `onDownloadStart`: DownloadListener передаёт приложению адрес
  файла, WebView не навигирует на download-контент (загрузку выполняет
  приложение).
