...
- `SimpleBrowserReopenPolicy.replaceCallbacksAndSettings` — сессия заменяется: привязываются обработчики нового запроса и применяются его настройки — пользовательский агент, режим адресной строки, режим отладки, приём кук (`enableCookiesAndroid`), доступ к файлам (`allowFileAccess`); начальные куки устанавливаются в хранилище и вступят в силу при следующей загрузке страницы. Страница не перезагружается.
...
Юнит-тесты плагина (валидация кук и userAgent, платформенный интерфейс, reopen-политики, маршрутизация событий):

```bash
cd simple_native_web_view_browser
flutter test
```

Интеграционные тесты (требуют устройства или симулятора; проверяют нативные сценарии — кастомные схемы, куки, повторное открытие):
